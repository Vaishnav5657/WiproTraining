...
export class FlightService {
  ...
  searchFlights(source: string, dest: string, date: string): Observable<Flight[]> {
    if (environment.useMocks) {
      ...
      const all: Flight[] = [
        { id: 1, source: 'HYD', dest: 'BLR', date, price: 3000, airline: 'IndiGo' },
        { id: 2, source: 'HYD', dest: 'BLR', date, price: 4000, airline: 'Air India' },
        { id: 3, source: 'HYD', dest: 'BLR', date, price: 6000, airline: 'Vistara' },
        { id: 4, source: 'DEL', dest: 'MUM', date, price: 5500, airline: 'Air India' },
        { id: 4, source: 'DEL', dest: 'MUM', date, price: 5800, airline: 'Air India' },
      ];
  ...
  initiateBooking(flightId: number, name: string, email: string, phone: string): Observable<Booking> {
    if (environment.useMocks) {
  ...
  bookFlight(flightId: number, name: string, email: string, phone: string): Observable<Booking> {
    return this.initiateBooking(flightId, name, email, phone);
  }
